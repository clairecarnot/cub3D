...
//		&& ((int)(game->posY + 0.9) > 1) && (game->posY < game->cols))
	{
		...
		dprintf(2, "4\n");
//		dprintf(2, "game->posX + cos(facing_direction - M_PI_2) * game->moveSpeed = %f\n", game->posX + cos(facing_direction - M_PI_2) * game->moveSpeed);
		if ((game->posX + cos(facing_direction - M_PI_2) * game->moveSpeed >= 1.1)
			&& (game->posX + cos(facing_direction - M_PI_2) * game->moveSpeed <= game->rows - 1.1))
			game->posX += cos(facing_direction - M_PI_2) * game->moveSpeed;
		if ((game->posY + sin(facing_direction - M_PI_2) * game->moveSpeed <= 1.1)
...
//		&& ((int)(game->posY + 0.9) > 1) && (game->posY < game->cols))
	{
		tmpX = (int) (game->posX + game->dirY * game->moveSpeed);
		tmpY = (int) (game->posY + game->dirX * game->moveSpeed);
		dprintf(2, "1\n");
		dprintf(2, "tmpX = %d\n", tmpX);
		dprintf(2, "tmpY = %d\n", tmpY);
		if (!game->map[tmpX])
		...
		if (game->map[tmpX][tmpY] == '1')
		{
			dprintf(2, "X plus grand\n");
			return ;
		}
		dprintf(2, "4\n");
		if ((game->posX + game->dirY * game->moveSpeed >= 1.1)
			&& (game->posX + game->dirY * game->moveSpeed <= game->rows - 1.1))
			game->posX += game->dirY * game->moveSpeed;
		if ((game->posY + game->dirX * game->moveSpeed >= 1.1)
			&& (game->posY + game->dirX * game->moveSpeed <= game->cols - 1.1))
